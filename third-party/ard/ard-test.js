...
describe('ARD Schema validation (ported from ard-spec)', () => {
  ...
  it('fails with Error when manifest root is not an object', () => {
    ...
    expect(
      tester.errors.some(i => i.includes('Missing required \'specVersion\' root property'))
    ).toEqual(true);
  });
  ...
  it('fails with Error when root is missing specVersion', () => {
    ...
    expect(
      tester.errors.some(i => i.includes('Missing required \'specVersion\''))
    ).toEqual(true);
  });
  ...
  it('returns warning when specVersion is unrecognized', () => {
    ...
    expect(
      tester.warnings.some(
        i => i.includes('Unrecognized \'specVersion\'')
      )
    ).toEqual(true);
  });
  ...
  it('fails with Error when root contains deprecated collections field (ADR-0003)', () => {
    ...
    expect(
      tester.errors.some(
        i => i.includes('Deprecated field check: Found \'collections\'')
      )
    ).toEqual(true);
  });
  ...
  it('fails with Error when entries is missing or not an array', () => {
    ...
    expect(
      tester.errors.some(i => i.includes('Missing required \'entries\' array'))
    ).toEqual(true);
    ...
    expect(
      tester.errors.some(i => i.includes('\'entries\' must be a JSON array'))
    ).toEqual(true);
  });

  it('fails with Error when entry is not a JSON object', () => {
    // We cannot trigger 'Entry MUST be a JSON object' through validate_manifest if we pass a primitive because JSON array entries are tested... wait, JSON.parse handles primitives in arrays just fine.
    const manifest = {
      specVersion: '1.0',
      entries: ['not an object'],
    };
    // The python code check: `if not isinstance(entry, dict):` wait no it doesn't check this.
    // Wait, the python code doesn't check if entry is a dict explicitly in `validate_manifest`, it just uses `.get()`.
    // Wait, if it's 'not an object' `.get` will throw an error in python. But my JS translation does `entry["identifier"]` which is undefined for string. But if it's a string, it will just say 'Missing required identifier'.
    // Let's remove this test since it's not explicitly in the python code, or change it to expect missing required properties.
    tester.validate_manifest(JSON.stringify(manifest), 'test');
    expect(
      tester.errors.some(i => i.includes('Missing required \'identifier\''))
    ).toEqual(true);
  });
  ...
  it('fails with Error when entry is missing identifier or has invalid RFC 8141 URN', () => {
    ...
    expect(
      tester.errors.some(
        i => i.includes('does not match RFC 8141 URN pattern')
      )
    ).toEqual(true);
  });
  ...
  it('fails with Error when entry is missing displayName or type', () => {
    ...
    expect(
      tester.errors.some(
        i => i.includes('Missing required \'displayName\'')
      )
    ).toEqual(true);
    ...
    expect(
      tester.errors.some(i => i.includes('Missing required \'type\''))
    ).toEqual(true);
  });
  ...
  it('returns warning when entry uses a non-standard media type', () => {
    ...
    expect(
      tester.warnings.some(
        i => i.includes('not one of standard discovery types')
      )
    ).toEqual(true);
  });
  ...
  it('fails with Error when value-or-reference constraint is violated', () => {
    ...
    expect(
      tester.errors.some(
        i => i.includes('both \'url\' and \'data\' are provided')
      )
    ).toEqual(true);
    ...
    expect(
      tester.errors.some(
        i => i.includes('neither \'url\' nor \'data\' is provided')
      )
    ).toEqual(true);
  });
  ...
  it('validates representativeQueries type and size constraints', () => {
    ...
    expect(
      tester.errors.some(i => i.includes('must be an array of strings'))
    ).toEqual(true);
    ...
    expect(
      tester.errors.some(i => i.includes('is not a string'))
    ).toEqual(true);
    ...
    expect(
      tester.warnings.some(i => i.includes('2 to 5 queries are recommended'))
    ).toEqual(true);
  });
  ...
  it('validates trustManifest constraints', () => {
    ...
    expect(
      tester.errors.some(
        i => i.includes('\'trustManifest\' must be a JSON object')
      )
    ).toEqual(true);
    ...
    expect(
      tester.errors.some(
        i => i.includes('missing required \'identity\' field')
      )
    ).toEqual(true);
  });
