...
(function(factory) {
  ...
  const Zone$1 = (function(global) {
    ...
    var notScheduled = 'notScheduled'; var scheduling = 'scheduling'; var scheduled = 'scheduled'; var running = 'running'; var canceling = 'canceling'; var unknown = 'unknown';
    var microTask = 'microTask'; var macroTask = 'macroTask'; var eventTask = 'eventTask';
    var patches = {};
    var _api = {
      symbol: __symbol__,
  ...
  function patchPrototype(prototype, fnNames) {
    ...
    const _loop_1 = function(i) {
      ...
      if (delegate) {
        const prototypeDesc = ObjectGetOwnPropertyDescriptor(prototype, name_1);
  ...
  function patchMethod(target, name, patchFn) {
    ...
    if (proto && (!(delegate = proto[delegateName]) || !proto.hasOwnProperty(delegateName))) {
      ...
      if (isPropertyWritable(desc)) {
        const patchDelegate_1 = patchFn(delegate, delegateName, name);
        proto[name] = function() {
  ...
  Zone.__load_patch('ZoneAwarePromise', function(global, Zone, api) {
    ...
    function patchThen(Ctor) {
      ...
      Ctor.prototype.then = function(onResolve, onReject) {
        const _this = this;
        const wrapped = new ZoneAwarePromise(function(resolve, reject) {
  ...
  function patchEventTarget(_global, apis, patchOptions) {
    ...
    const invokeTask = function(task, target, event) {
      ...
      if (options && typeof options === 'object' && options.once) {
        ...
        const delegate_1 = task.originalDelegate ? task.originalDelegate : task.callback;
    ...
    function patchEventTargetMethods(obj, patchOptions) {
      ...
      const makeAddListener = function(nativeListener, addSource, customScheduleFn, customCancelFn, returnTarget, prepend) {
  ...
  function filterProperties(target, onProperties, ignoreProperties) {
    ...
    const tip = ignoreProperties.filter(function(ip) {
  ...
  function propertyDescriptorPatch(api, _global) {
    ...
    if (isBrowser) {
      const internalWindow_1 = window;
      const ignoreErrorProperties = isIE() ? [{target: internalWindow_1, ignoreProperties: ['error']}] : [];
      ...
      const HTMLMarqueeElement_1 = internalWindow_1['HTMLMarqueeElement'];
      if (HTMLMarqueeElement_1) {
        patchFilteredProperties(HTMLMarqueeElement_1.prototype, marqueeEventNames, ignoreProperties);
      }
      const Worker_1 = internalWindow_1['Worker'];
      if (Worker_1) {
  ...
  function eventTargetLegacyPatch(_global, api) {
    const _a = api.getGlobalObjects(); const eventNames = _a.eventNames; const globalSources = _a.globalSources; const zoneSymbolEventNames = _a.zoneSymbolEventNames; const TRUE_STR = _a.TRUE_STR; const FALSE_STR = _a.FALSE_STR; const ZONE_SYMBOL_PREFIX = _a.ZONE_SYMBOL_PREFIX;
  ...
  function apply(api, _global) {
    const _a = api.getGlobalObjects(); const ADD_EVENT_LISTENER_STR = _a.ADD_EVENT_LISTENER_STR; const REMOVE_EVENT_LISTENER_STR = _a.REMOVE_EVENT_LISTENER_STR;
  ...
  function propertyDescriptorLegacyPatch(api, _global) {
    const _a = api.getGlobalObjects(); const isNode = _a.isNode; const isMix = _a.isMix;
    if (isNode && !isMix) {
      return;
    }
  ...
  function canPatchViaPropertyDescriptor(api, _global) {
    const _a = api.getGlobalObjects(); const isBrowser = _a.isBrowser; const isMix = _a.isMix;
    if ((isBrowser || isMix) &&
    ...
    if (xhrDesc) {
      ...
      const SYMBOL_FAKE_ONREADYSTATECHANGE_1 = api.symbol('fake');
      api.ObjectDefineProperty(XMLHttpRequestPrototype, ON_READY_STATE_CHANGE, {
        enumerable: true,
        configurable: true,
        get: function() {
  ...
  function patchViaCapturingAllTheEvents(api) {
    const eventNames = api.getGlobalObjects().eventNames;
    const unboundKey = api.symbol('unbound');
    const _loop_4 = function(i) {
      const property = eventNames[i];
      const onproperty = 'on' + property;
      self.addEventListener(property, function(event) {
        let elt = event.target; let bound; let source;
        if (elt) {
            ...
            patchMethod(window, setName, function(delegate) {
              return function(self, args) {
                if (typeof args[0] === 'function') {
                  ...
                  const task = scheduleMacroTaskWithCurrentZone(setName, args[0], options_1, scheduleTask, clearTask);
                  if (!task) {
                    return task;
                  }
  ...
  function eventTargetPatch(_global, api) {
    ...
    const _a = api.getGlobalObjects(); const eventNames = _a.eventNames; const zoneSymbolEventNames = _a.zoneSymbolEventNames; const TRUE_STR = _a.TRUE_STR; const FALSE_STR = _a.FALSE_STR; const ZONE_SYMBOL_PREFIX = _a.ZONE_SYMBOL_PREFIX;
    //  predefine all __zone_symbol__ + eventName + true/false string
    for (let i = 0; i < eventNames.length; i++) {
  ...
  Zone.__load_patch('blocking', function(global, Zone) {
    ...
    for (let i = 0; i < blockingMethods.length; i++) {
      const name_2 = blockingMethods[i];
  ...
  Zone.__load_patch('XHR', function(global, Zone) {
    ...
    function patchXHR(window) {
      ...
      if (!oriAddListener) {
        const XMLHttpRequestEventTarget_1 = window['XMLHttpRequestEventTarget'];
        if (XMLHttpRequestEventTarget_1) {
          const XMLHttpRequestEventTargetPrototype = XMLHttpRequestEventTarget_1.prototype;
      ...
      function scheduleTask(task) {
        ...
        const newListener = target[XHR_LISTENER] = function() {
