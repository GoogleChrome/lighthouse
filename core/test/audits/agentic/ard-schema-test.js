...
describe('ARD Schema Audit', () => {
  ...
  it('returns a warning when representativeQueries is missing (Lighthouse best practice)', () => {
    ...
    expect(result.details.items[0].severity).toBeDisplayString('Warning');
    expect(result.details.items[0].issue).toBeDisplayString(
      'Missing `representativeQueries`. Providing examples significantly improves discoverability.'
    );
  });
});
