...
describe('Metrics: LCP from all frames', () => {
  ...
  it('should throw for predicted value', async () => {
    ...
    const resultPromise = LargestContentfulPaintAllFrames.request(
      ...
      {gatherContext, trace: traceAllFrames, devtoolsLog: devtoolsLogAllFrames, settings, URL: getURLArtifactFromDevtoolsLog(devtoolsLogAllFrames), simulator: null},
      context);
  ...
  it('should fail to compute an observed value for old trace', async () => {
    ...
    const resultPromise = LargestContentfulPaintAllFrames.request(
      // eslint-disable-next-line max-len
      {gatherContext, trace: invalidTrace, devtoolsLog: invalidDevtoolsLog, settings, URL: getURLArtifactFromDevtoolsLog(invalidDevtoolsLog), simulator: null},
      context
    );
  ...
  it('should use main frame LCP if no other frames', async () => {
    ...
    const result = await LargestContentfulPaintAllFrames.request(
      // eslint-disable-next-line max-len
      {gatherContext, trace: traceMainFrame, devtoolsLog: devtoolsLogMainFrame, settings, URL: getURLArtifactFromDevtoolsLog(devtoolsLogMainFrame), simulator: null},
      context
    );
